...
如果一开始他们的定位就换一种，a负责组装，b负责制造。无论a造了个什么东西（依赖），随意丢给b（注入到b提供的容器）就好了，让b组装，后来他们就愉快合作，也无惧任何需求了。这就是依赖注入带来的显著效果。我们平时可以在一些地方看见它的影子：amd模块化方案、react用props展示组件、angular1大量使用、react hook等
